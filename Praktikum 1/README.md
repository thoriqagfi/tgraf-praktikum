...
Jika `low` tidak sama dengan `high`, hitung nilai tengah (`mid`) dari rentang yang sedang diproses.
Periksa apakah indeks `2 * pos + 2` melebihi panjang pohon segmen (`len(tree)`). Jika ya, perlu memperluas panjang array `tree` untuk menampung posisi tersebut.
Rekursif memanggil `buildTree` untuk anak kiri (posisi `2 * pos + 1`) dengan rentang dari `low` hingga `mid`, dan untuk anak kanan (posisi `2 * pos + 2`) dengan rentang dari `mid + 1` hingga `high`.
...
p = [(arr[i], i) for i in range(n)]
p.sort(key=lambda x: (x[0], -x[1]))
len_tree = 2 \*_ (math.ceil(math.sqrt(n)) + 1) - 1
tree = [0] _ len_tree
for i in range(n):
buildTree(tree, 0, 0, n - 1, p[i][1],
findMax(tree, 0, 0, n - 1, 0, p[i][1]) + 1)
